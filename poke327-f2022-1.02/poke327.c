...
typedef struct map
{
  ...
  int8_t n, s, e, w;
  pair_t pc;
  uint32_t hiker_map[MAP_Y][MAP_X];
  uint32_t rival_map[MAP_Y][MAP_X];
} map_t;
...
{
  map_t *world[WORLD_SIZE][WORLD_SIZE]; // 2d array of pointers to map, map contains
  pair_t cur_idx;
  map_t *cur_map;
} world_t;
...
place_pc()
{
  // extract current map from world
  map_t *m = world.world[world.cur_idx[0]][world.cur_idx[1]];

  srand(time(0));
  int x = rand() % 80;
  int y = rand() % 21;
  while (m->map[y][x] == ter_boulder || m->map[y][x] == ter_tree || m->map[y][x] == ter_mart || m->map[y][x] == ter_center || m->map[y][x] == ter_mountain)
  {
    x = rand() % 80;
    y = rand() % 21;
  }
  m->pc[0] = x;
  m->pc[1] = y;
  m->map[y][x] = ter_player;
}

static int32_t path_cmp(const void *key, const void *with)
{
  return ((path_t *)key)->dist - ((path_t *)with)->dist;
}
...
static void dijkstra_path(map_t *m, pair_t from, pair_t to)
{
  ...
  for (y = 0; y < MAP_Y; y++)
  {
    for (x = 0; x < MAP_X; x++)
    {
      path[y][x].dist = INT_MAX;
    }
  }

  path[from[dim_y]][from[dim_x]].dist = 0;
  ...
  while ((p = heap_remove_min(&h)))
  {
    ...
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]].hn) &&
        (path[p->pos[dim_y] - 1][p->pos[dim_x]].dist >
         ((p->dist + heightpair(p->pos)) *
    ...
    if ((path[p->pos[dim_y]][p->pos[dim_x] - 1].hn) &&
        (path[p->pos[dim_y]][p->pos[dim_x] - 1].dist >
         ((p->dist + heightpair(p->pos)) *
...
int32_t hiker_penalty(map_t *m, uint16_t pos[2])
{
  int x, y;
  x = pos[0];
  y = pos[1];
  terrain_type_t terrain = mapxy(x, y);
  // covers exits
...
int travel_cost(int32_t dist, int32_t hiker_penalty)
{
  printf("dist: %d, hikerpenalty: %d\n", dist, hiker_penalty);

  if (dist == INT32_MAX || hiker_penalty == INT32_MAX)
  {
    return INT32_MAX;
  }
  return dist + hiker_penalty;
}

void dijkstra_hiker_2_pc(map_t *m)
{
  ...
  // initialize dist attribute for each cell in path[][] to infinity
  for (y = 0; y < MAP_Y; y++)
  {
    for (x = 0; x < MAP_X; x++)
    {
  ...
  // distance of traveling from PC to PC = 0
  path[m->pc[dim_y]][m->pc[dim_x]].dist = 0;
  ...
  for (y = 1; y < MAP_Y - 1; y++)
  {
    ...
    {
      uint16_t pos[2] = {x, y};
      if (hiker_penalty(m, pos) != INT_MAX)
      {
        path[y][x].hn = heap_insert(&h, &path[y][x]);
      }
  ...
  while ((p = heap_remove_min(&h)))
  {
    m->hiker_map[p->pos[dim_y]][p->pos[dim_x]] = p->dist;
    // printf("%d", p->dist);
    return;
    // (-1, -1)
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].hn) &&
        (path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].dist > (travel_cost(p->dist, hiker_penalty(m, p->pos)))))
    {
      path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].dist = travel_cost(p->dist, hiker_penalty(m, p->pos));
      printf("%d\n", travel_cost(p->dist, hiker_penalty(m, p->pos)));
    ...
    // (0, -1)
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]].hn) &&
        (path[p->pos[dim_y] - 1][p->pos[dim_x]].dist > (travel_cost(p->dist, hiker_penalty(m, p->pos)))))
    {
      path[p->pos[dim_y] - 1][p->pos[dim_x]].dist = travel_cost(p->dist, hiker_penalty(m, p->pos));
      path[p->pos[dim_y] - 1][p->pos[dim_x]].from[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y] - 1][p->pos[dim_x]].from[dim_x] = p->pos[dim_x];
    ...
    // (-1, 0)
    if ((path[p->pos[dim_y]][p->pos[dim_x] - 1].hn) &&
        (path[p->pos[dim_y]][p->pos[dim_x] - 1].dist > (travel_cost(p->dist, hiker_penalty(m, p->pos)))))
    {
      path[p->pos[dim_y]][p->pos[dim_x] - 1].dist = travel_cost(p->dist, hiker_penalty(m, p->pos));
      path[p->pos[dim_y]][p->pos[dim_x] - 1].from[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y]][p->pos[dim_x] - 1].from[dim_x] = p->pos[dim_x];
    ...
    // (1, 0)
    if ((path[p->pos[dim_y]][p->pos[dim_x] + 1].hn) &&
        (path[p->pos[dim_y]][p->pos[dim_x] + 1].dist > (p->dist + hiker_penalty(m, p->pos))))
    {
      path[p->pos[dim_y]][p->pos[dim_x] + 1].dist = travel_cost(p->dist, hiker_penalty(m, p->pos));
      path[p->pos[dim_y]][p->pos[dim_x] + 1].from[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y]][p->pos[dim_x] + 1].from[dim_x] = p->pos[dim_x];
...
int main(int argc, char *argv[])
{
  ...
  place_pc();
  dijkstra_hiker_2_pc(world.cur_map);
